...
export function registerAuthRoutes(app: Express): void {
  ...
  app.get("/callback", async (req, res) => {
    ...
    if (!clientId || !clientSecret || !redirectUri) {
      res.status(500).send("Missing OAuth environment variables"); return;
    }
    ...
    try {
      ...
      if (!tokenRes.ok) {
        res.status(500).send(`Token exchange failed: ${await tokenRes.text()}`); return;
      }
      ...
      res.send(`
        <html><body style="font-family:monospace;padding:40px;background:#0a0a0a;color:#00ff88">
          <h2>✅ Authorized</h2>
          <p>Tokens saved. Daily cron active — syncs at 6am ET.</p>
          <p>Add to Railway environment variables if not already set:</p>
  ...
  app.get("/status", (_req, res) => {
    res.json({
      status: "ok",
      authorized: !!process.env.WHOOP_REFRESH_TOKEN,
      message: process.env.WHOOP_REFRESH_TOKEN
        ? "Authorized. Daily sync active at 6am ET."
